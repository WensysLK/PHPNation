...
ON enjaz_details.CleintId = contract_details.applicationID WHERE enjaz_details.EnjazStatus='processing' AND enjaz_details.softdeletestatus=1";
    ...
    $resenjaz = mysqli_query($conn, $sqenjaz);

    if ($resenjaz == true) {
        $count_rows = mysqli_num_rows($resenjaz);
        $num = 1;
    ...
    <tbody>
        <?php
            while ($row = mysqli_fetch_assoc($resenjaz)) {
   ...
   data-bs-toggle="modal" data-animation="effect-slide-in-bottom">
    ...
    <?php 
            ...
            echo "<div class='alert alert-primary mt-2' role='alert'> No Processing Enjaz!</div>";
        }
    } ?>
</table>
